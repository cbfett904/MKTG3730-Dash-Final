...
function renderKpis(rows) {
  ...
  const pairs = [
    ["UP3 Followers", "UP3 Followers"],
    ["Total Growth", "Follower Growth"],
    ["UP2→UP3", "UP2→UP3 Growth"],
    ["UP3 Reach", "UP3 Reach"],
  ];
...
function buildComparisonControls(dataRows) {
  ...
  if (state.selectedMetrics.size === 0 && metrics.length) {
    const defaultMetrics = ["UP2 Followers", "Follower Growth"].filter(m => metrics.includes(m));
...
function renderFbIgChart(dataRows) {
  ...
  brands.forEach(b => {
    ...
    const igFollowers = igRow ? toNumber(igRow["UP3 Followers"]) : 0;
    const fbFollowers = fbRow ? toNumber(fbRow["UP3 Followers"]) : 0;
...
function buildInfluenceControls(dataRows) {
  ...
  const xDefault = metrics.find(m => normalize(m).includes("posts")) || metrics[0];
...
async function loadData() {
  try {
    ...
    // Build brand-name dropdown for the Team filter
    const brandNames = uniqSorted(state.dataRows.map(r => r["Brand Page"]));
    const teamSel = el("filterTeam");
    teamSel.innerHTML = "";
    const o0 = document.createElement("option");
    o0.value = ""; o0.textContent = "All brands";
    teamSel.appendChild(o0);
